...
class QuotationController extends Controller {
	...
	public function paymetQuotation(Request $request) 
       ...
       $quotation =    $quotation->where('o.status','=',1)
          ...
          if($status > 0){
             $quotationsQuery = $quotation->where('quotations.quotations_status','=',$status);   
            }
            
             if($agent > 0){
             $quotationsQuery = $quotationsQuery->where('quotations.fkUser','=',$agent);   
            }
           
            
            if(!empty($fechStart)){
             $quotationsQuery = $quotationsQuery->whereDate('quotations.register_day','>=',$fechStart);   
            }
            
            if(!empty($fechFinish)){
             $quotationsQuery = $quotationsQuery->whereDate('quotations.register_day','<=',$fechFinish);   
            }

            $quotationsQuery =  $quotationsQuery->get();
